...
def main():
    ...
    for current_clip_begin in range(0, video_length, clip_len):
        current_clip_end = min(current_clip_begin + clip_len, video_length)
        ...
        if file_extension in VIDEO_EXTENSIONS:  # input is a video file
            video_reader = mmcv.VideoReader(args.input_dir)
            inputs = []
            for frame in video_reader[current_clip_begin:current_clip_end]:
